...
function draw_button(x,y,color,message,sizeX,sizeY)
    sizeX = sizeX or 7
    sizeY = sizeY or 2
...
function draw_menu()
    ...
    draw_button(menu_map[1].x,menu_map[1].y,
    colors.gray, " Menu")
end
draw_menu()
...
while true do
    ...
    for i,button in ipairs(button_map) do
        if x >= button.x and x <= button.x+7 
        and y >= button.y and y <= button.y+2 then
